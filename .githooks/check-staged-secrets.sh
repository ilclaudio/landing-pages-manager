...
echo "[KKLPM pre-commit] Running secret scan..." >&2
...
for pattern in "${patterns[@]}"; do
	if printf '%s\n' "$added_lines" | grep -E -i -n "$pattern" >/tmp/kklpm_secret_scan_matches.$$ 2>/dev/null; then
		if [ "$found" -eq 0 ]; then
			echo "[KKLPM pre-commit] Potential secret detected in staged changes:" >&2
			found=1
		fi
		cat /tmp/kklpm_secret_scan_matches.$$ >&2
	fi
	rm -f /tmp/kklpm_secret_scan_matches.$$ >/dev/null 2>&1 || true
